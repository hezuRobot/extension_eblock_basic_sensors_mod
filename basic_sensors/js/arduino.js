...
(function(ext) {
  ...
  ext.tocaRTTL = function(melodia,pin){
    var deviceId = 65;
    melodia = melodia.toString();
    var j = (((melodia.length)-(melodia.length % 42))/42);  //necesito saber cuantos paquetes enteros tengo que enviar
    if (j != 0){
      for(var m=1;m<(j+1);++m){
        runPackage(deviceId,pin,42,m,string2array(melodia.substring((m-1)*42,(m)*42)));
      }
    }
    runPackage(deviceId,pin,melodia.length % 42,0,string2array(melodia.substring((j)*42,(j)*42+(melodia.length % 42))));
    // este es el paquete final que sera menor que el máximo permitido
  };
